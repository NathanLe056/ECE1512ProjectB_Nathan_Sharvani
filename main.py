...
from model import ABMIL, ABMIL_NoAttention
...
def main(args):
    ...
    # choose model variant from config (default: ABMIL)
    if hasattr(conf, 'model_variant') and conf.model_variant == 'ABMIL_NoAttention':
        model = ABMIL_NoAttention(conf=conf)
    else:
    ...
    print('Configuration:')
    pprint.pprint(conf)
    # select loss by config (default: CrossEntropy)
    if hasattr(conf, 'loss_type') and conf.loss_type == 'focal':
        criterion = FocalLoss(alpha=getattr(conf, 'focal_alpha', 1.0),
    ...
    csv_path = os.path.join('results', f"{config_name}_log.csv")
    csv_file = open(csv_path, 'w', newline='')
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(['epoch','val_auc','val_acc','val_f1','val_loss','test_auc','test_acc','test_f1','test_loss'])
    ...
    for epoch in range(conf.train_epoch):
        ...
        if val_f1 + val_auc > best_state['val_f1'] + best_state['val_auc']:
            ...
            ckpt_path = os.path.join('results', f"{config_name}_best.pth")
            torch.save({'model_state_dict': model.state_dict(),
                        'optimizer_state_dict': optimizer.state_dict(),
                        'epoch': epoch,
                        'conf': vars(conf)}, ckpt_path)
